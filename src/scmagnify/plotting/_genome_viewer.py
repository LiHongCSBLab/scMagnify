...
from typing import TYPE_CHECKING, Literal, Union, Optional, List, Dict
...
from scmagnify.utils import _get_data_modal, _get_X, _pyranges_from_strings, _pyranges_to_strings, d, inject_docs
...
class GenomeViewer:
    ...
    def __init__(
        ...
        gtf: Optional[Union[pr.PyRanges, str]] = None,
    ...
    def _preprocess_metadata(self):
        ...
        if f"{self.cluster}_colors" in self.adata.uns:
            self.colors = pd.Series(self.adata.uns[f"{self.cluster}_colors"], index=self.adata.obs[self.cluster].cat.categories)
        else:
            self.colors = pd.Series(sns.color_palette("Set2", len(self.cluster_order)).as_hex(), index=self.cluster_order)
    ...
    def _load_and_process_gtf(self, gtf: Optional[Union[pr.PyRanges, str]]) -> pr.PyRanges:
    ...
    def _process_links(self, links: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
        """Pre-processes the links DataFrame for plotting."""
        if links is None: return None
        logg.info("Processing links...")
        if not isinstance(links, pd.DataFrame): raise TypeError("`links` must be a pandas DataFrame.")
        if "gene" in links.columns and "cor" in links.columns:
            ...
            raise ValueError("`links` DataFrame must contain either ['gene', 'cor'] or ['start', 'end', 'cor'] columns.")

    def _compute_coverage(self, region: str, barcodes: pd.Series, out_prefix: str, smooth: Optional[int], normalize: bool, frag_type: str, nfrags_key: str = "nFrags") -> pd.Series:
        ...
        with open(f"{out_prefix}.bed", "w") as bed_file:
            for sample in self.fragment_files:
                ...
                try: records = tb.querys(region)
                except tabix.TabixError:
                    logg.warning(f"Region '{region}' not found in sample '{sample}'. Skipping.")
                    continue
                for record in records:
                    if record[3] not in barcodes[sample]: continue
                    frag_len = int(record[2]) - int(record[1])
                    if (frag_type == "NFR" and frag_len > 145) or (frag_type == "NUC" and frag_len <= 145): continue
                    bed_file.write(f"{record[0]}\t{record[1]}\t{record[2]}\n")
        with open(f"{out_prefix}.region.bed", "w") as bed_file: bed_file.write(region.replace(":", "\t").replace("-", "\t") + "\n")
        with open(f"{out_prefix}.coverage.bed", "w") as out_file: subprocess.call(["bedtools", "coverage", "-a", f"{out_prefix}.region.bed", "-b", f"{out_prefix}.bed", "-d"], stdout=out_file)
        try:
        ...
        except (pd.errors.EmptyDataError, IndexError): return pd.Series(dtype='float64')
        if smooth: coverage = coverage.rolling(smooth, center=True).mean().fillna(coverage.iloc[smooth])
        if normalize:
            n_frags = sum(self.adata.obs[nfrags_key][(self.adata.obs_names.str.contains(sample)) & (self.adata.obs["FragSample"].isin(barcodes[sample]))].sum() for sample in barcodes.index)
    ...
    def _plot_track_coverage(self, coverage, track_name, ax, color, rasterized=False, **kwargs):
        ...
        if y_font: ax.tick_params(axis="y", labelsize=y_font)
        values = coverage.copy(); values[values <= kwargs.get("min_coverage", 0)] = 0
        ...
        if "ylim" in kwargs and kwargs["ylim"] is not None: ax.set_ylim(kwargs["ylim"])
    ...
    def _plot_track_bed(self, plot_peaks, ax, facecolor):
        ...
        ax.set_ylim([-1, 1]); ax.set_yticks([]); ax.axes.get_xaxis().set_visible(False)
    ...
    def _plot_motif_hits(self, motif_hits, motif, ax, facecolor):
        ...
        if ax is None: ax = plt.gca()
        if motif_hits.empty:
            ax.set_yticks([]); sns.despine(ax=ax)
            return

        rects = [Rectangle((s, -0.45), e - s, 0.9) for s, e in zip(motif_hits.Start, motif_hits.End)]
        ax.add_collection(PatchCollection(rects, facecolor=facecolor, edgecolor="black"))
    ...
    def _plot_track_links(self, links, ax, color):
        ...
        for _, row in links.iterrows():
            ...
            arc = Arc(
                    ...
                    lw=1.25, color=palette(row["cor"])
                )
            ax.add_patch(arc)
        ax.set_ylim([min(-abs((links["end"] + links["start"]) / 2 - links["start"])) - 100, 0])
        ax.set_axis_off()

    def _plot_side_plot(self, plot_data, ax, color, plot_type, is_first, is_last, y_font, side_genes, xlabel="Expression"):
        ...
        ax.set_yticks([]); ax.set_ylabel(''); ax.set_xlabel('')
        ...
        if is_last:
            ...
            if plot_data.expression.min() >= 0:
                ...
                ax.set_xticklabels([f'0.0', f'{x_max:.1f}'], fontsize=y_font or 10)
    ...
    def add_motifs(self, 
                   ...
                   motif_score: Optional[pd.DataFrame] = None,
        ...
        """
        from scmagnify.tools._motif_scan import parse_pfm, MOTIF_DIR
        ...
        if tfs is not None:
            ...
            candidate_motifs.extend([
                motif for motif in pfm_objects 
                if any(p.search(motif.name) for p in tf_patterns)
            ])
        ...
        if target_organism:
            ...
            factor_df = pd.DataFrame(
                [motif.name for motif in self.motifs], 
                columns=['factor']
            )
        
            trans_df = dc.op.translate(factor_df,
                            'factor', 
                            target_organism,
                            verbose=True)
            
            if trans_df.shape[0] < factor_df.shape[0]:
                logg.warning(f"{factor_df.shape[0] - trans_df.shape[0]} factors could not be translated to the target organism. ")
    ...
    def plot(
        ...
        highlight_peaks: Optional[Union[pr.PyRanges, List[str]]] = None,
        ...
        with mpl.rc_context(rc_params):
            ...
            if anchor_gene:
                ...
                if tss.empty: raise ValueError(f"Gene {anchor_gene} not found in GTF.")
            ...
            coverages = {k: self._compute_coverage(region, self.barcode_groups[k], "/tmp/", smooth, normalize, frag_type, nfrags_key) for k in _cluster_order}
            ...
            genes_to_plot = self.gtf[self.gtf.gene_name == anchor_gene] if anchor_gene else self.gtf.intersect(pr_region)

            # Step 5: Setup Figure and GridSpec
            # Step 5: Setup Figure and GridSpec
            n_cov_rows = len(coverages) if not collapsed else 1
            _peak_groups_to_plot = self.peaks
            n_peak_rows = len(_peak_groups_to_plot) if _peak_groups_to_plot is not None and not _peak_groups_to_plot.empty else 0
            ...
            track_rows = n_cov_rows + n_peak_rows + n_motif_hits_rows + n_link_rows + n_gene_rows + n_spacer_rows + n_motif_plot_rows

            base_ratios_list = []
            if not collapsed: base_ratios_list.extend(np.repeat(plot_cov_size, n_cov_rows))
            else: base_ratios_list.append(plot_cov_size * 4)
            if n_peak_rows > 0: base_ratios_list.extend(np.repeat(plot_bed_size, n_peak_rows))
            if n_motif_hits_rows > 0: base_ratios_list.extend(np.repeat(plot_bed_size, n_motif_hits_rows))
            if n_link_rows > 0: base_ratios_list.append(plot_link_size)
            if n_gene_rows > 0: base_ratios_list.append(plot_bed_size * 2)
            
            # Insert the spacer's height ratio at the correct position
            if n_spacer_rows > 0: base_ratios_list.append(spacer_size)
                
            if n_motif_plot_rows > 0: base_ratios_list.extend(np.repeat(plot_motif_size, n_motif_plot_rows))
            ...
            for i, cluster_name in enumerate(_cluster_order):
                if cluster_name not in coverages: continue
                ax_cov = fig.add_subplot(gs[plot_idx, 0]); all_left_axes.append(ax_cov)
                last_ax = ax_cov
                ax_cov.set_xlim([start, end])
                coverage = coverages[cluster_name]
                self._plot_track_coverage(coverage, cluster_name, ax_cov, self.colors[cluster_name], y_font=y_font, min_coverage=min_coverage, rasterized=rasterize_coverage)
                ...
                rect = Rectangle((-0.1, 0), 0.03, rect_height + 0.22, facecolor=self.colors[cluster_name],
                                transform=ax_cov.transAxes, edgecolor='black', linewidth=0.5, clip_on=False)
                ax_cov.add_patch(rect)
                text_y_center = 0 + (rect_height / 2.0)
                ax_cov.text(-0.12, text_y_center, cluster_name, transform=ax_cov.transAxes, 
                            ha='right', va='center', fontsize=y_font or 10)
                
                ax_cov.set_ylabel(""); ax_cov.set_xticks([]); ax_cov.spines['bottom'].set_visible(False)
                
                if side_genes:
                    ax_side = fig.add_subplot(gs[plot_idx, 1]); all_right_axes.append(ax_side)
                    cluster_df = df_melt[df_melt[self.cluster] == cluster_name]
                    self._plot_side_plot(cluster_df, ax_side, self.colors[cluster_name], side_plot_type, 
                                        is_first=(i==0), is_last=(i==len(_cluster_order)-1), y_font=y_font, side_genes=side_genes,
            ...
            if hasattr(self, "peaks") and not self.peaks.empty:
                for name, peaks_pr in self.peaks.items():
                    ...
                    if side_genes: fig.add_subplot(gs[plot_idx, 1]).set_axis_off()
                    plot_idx += 1
            ...
            if hasattr(self, "motif_score") and self.motif_score is not None and not self.motif_score.empty:
                ...
                if self.motifs:
                    for motif in self.motifs:
                        ...
                        if motif_hits_fil.empty: continue
            ...
            if peaks_to_highlight is not None and not peaks_to_highlight.empty:
                ...
                for ax in all_left_axes:
                    ...
                    for s, e in zip(peaks_to_highlight.Start, peaks_to_highlight.End):
            ...
            if last_ax:
                last_ax.axes.get_xaxis().set_visible(True); last_ax.spines['bottom'].set_visible(True)
                locs = [start, end]
            ...
            # You could optionally add an invisible axis here, but it's not necessary
                ax_spacer = fig.add_subplot(gs[plot_idx, :]); ax_spacer.set_axis_off()
                plot_idx += 1
            ...
            if n_motifs > 0 and show_motif_logos:
                ...
                for i in range(n_motif_plot_rows):
                    ...
                    nested_gs = gridspec.GridSpecFromSubplotSpec(1, motifs_per_row, subplot_spec=gs_row_spec, wspace=0.4)
                    ...
                    for j, motif in enumerate(motifs_in_row):
                        ...
                        seqlogo(pfm=motif, ax=ax_motif, data_type="probability", title=None) # Hide motif xlabel to avoid clutter
