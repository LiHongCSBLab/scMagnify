...
def _validate_and_load_fonts(font_list: list[str], font_dir: str = "data/fonts") -> list[str]:
    """
    Validate if fonts are available in the system and load from package if not.

    Parameters
    ----------
    font_list : list[str]
        List of font names to validate (e.g., ["Arial", "Helvetica"]).
    ...
    -------
    list[str]
        Updated list of available font names.
    """
    ...
    # Try to locate the font directory in the package
    try:
        from importlib.resources import files
        font_path = files("scmagnify") / font_dir
        if not font_path.exists():
            logg.warning(f"Font directory {font_path} does not exist.")
            return [font for font in font_list if font in available_fonts]
    ...
    for font in font_list:
        if font in available_fonts:
            ...
            # Try to load font from package
            font_file = font_path / f"{font}.ttf"
            if font_file.exists():
                try:
                    fm.fontManager.addfont(str(font_file))
                    available_fonts.add(font)
                    valid_fonts.append(font)
                    package_fonts.append(font)
                    logg.debug(f"Loaded font {font} from {font_file}")
                except Exception as e:
                    logg.warning(f"Failed to load font {font} from {font_file}: {e}")
            else:
                logg.warning(f"Font {font} not found in system or {font_file}")

    # Ensure at least one fallback font
    if not valid_fonts:
        valid_fonts.append("sans-serif")
...
def savefig_or_show(
    ...
    -----
        ...
        - The figure is saved in the directory specified by `settings.figdir`.
    ...
    if save:
        ...
        filename = f"{settings.figdir}{writekey}.{ext}"
        if "/" in writekey:
...
def _format_title(title: str) -> str:
    """
    Format a title by replacing underscores with spaces, applying title case,
    and converting text within parentheses to lowercase.

    Parameters
    ----------
    title : str
        The input title string (e.g., "network_score (GRN)").

    Returns
    -------
    str
        Formatted title (e.g., "Network Score (grn)").
    """
