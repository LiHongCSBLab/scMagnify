...
from typing import TYPE_CHECKING, Union, Optional, Tuple, List, Literal
...
from scmagnify.plotting._utils import savefig_or_show, _setup_rc_params, _label_features
...
def dumbbellplot(
    data: Union[AnnData, MuData, GRNMuData, pd.DataFrame],
    col1: str,
    col2: str,
    modal: Optional[Literal["GRN", "RNA", "ATAC"]] = "GRN",
    ...
    **kwargs,
    ...
    with mpl.rc_context(rc_params):
        if label1 is None: label1 = col1
        if label2 is None: label2 = col2
        ...
        for _, row in dfc.iterrows():
            ax.plot([row['sort'], row['sort']], [row[col1], row[col2]], color='gray', alpha=0.5, zorder=1, linewidth=0.8)
        ...
        if threshold is not None:
            ...
            sns.scatterplot(x='sort', y=col2, data=dfc_left[dfc_left[col2] >= threshold], color=color2, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col2, data=dfc_left[dfc_left[col2] < threshold], color=below_color, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)
            
            # On the right side, col1 is on the bottom
            sns.scatterplot(x='sort', y=col1, data=dfc_right[dfc_right[col1] >= threshold], color=color1, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col1, data=dfc_right[dfc_right[col1] < threshold], color=below_color, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)

            # --- Plot the TOP layers second (zorder=3) ---
            # On the left side, col1 is on top
            sns.scatterplot(x='sort', y=col1, data=dfc_left[dfc_left[col1] >= threshold], color=color1, label=label1, ax=ax, zorder=3, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col1, data=dfc_left[dfc_left[col1] < threshold], color=below_color, label='_nolegend_', ax=ax, zorder=3, s=15, **kwargs)

            # On the right side, col2 is on top
            sns.scatterplot(x='sort', y=col2, data=dfc_right[dfc_right[col2] >= threshold], color=color2, label=label2, ax=ax, zorder=3, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col2, data=dfc_right[dfc_right[col2] < threshold], color=below_color, label='_nolegend_', ax=ax, zorder=3, s=15, **kwargs)

        else:
            # Plot bottom layers (zorder=2)
            sns.scatterplot(x='sort', y=col2, data=dfc_left, color=color2, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col1, data=dfc_right, color=color1, label='_nolegend_', ax=ax, zorder=2, s=15, **kwargs)

            # Plot top layers (zorder=3)
            sns.scatterplot(x='sort', y=col1, data=dfc_left, color=color1, label=label1, ax=ax, zorder=3, s=15, **kwargs)
            sns.scatterplot(x='sort', y=col2, data=dfc_right, color=color2, label=label2, ax=ax, zorder=3, s=15, **kwargs)
    ...
    if selected_genes:
        ...
        if label_anchor == 'midpoint':
            ...
            y_coords = dfc_for_labels.loc[labels_to_plot].apply(lambda r: r[col1] if abs(r[col1]) >= abs(r[col2]) else r[col2], axis=1)
        
        labels_to_plot = [g for g in genes_to_label if g in x_coords.index]
        _label_features(ax=ax, x_coords=x_coords, y_coords=y_coords, 
                        labels_to_plot=labels_to_plot, font_scale=font_scale)
        ...
        if labels_left:
            ...
            if label_anchor == 'max_abs':
                y_coords_left = dfc_left.loc[labels_left].apply(lambda r: r[col1] if abs(r[col1]) >= abs(r[col2]) else r[col2], axis=1)
            elif label_anchor == 'max':
                y_coords_left = dfc_left.loc[labels_left].apply(lambda r: r[col1] if r[col1] >= r[col2] else r[col2], axis=1)
            else: # midpoint
                y_coords_left = (dfc_left.loc[labels_left, col1] + dfc_left.loc[labels_left, col2]) / 2
            
            _label_features(ax=ax, x_coords=x_coords_left, y_coords=y_coords_left, 
                            labels_to_plot=labels_left, font_scale=font_scale)
        ...
        if labels_right:
            ...
            if label_anchor == 'max_abs':
                y_coords_right = dfc_right.loc[labels_right].apply(lambda r: r[col1] if abs(r[col1]) >= abs(r[col2]) else r[col2], axis=1)
            elif label_anchor == 'max':
                y_coords_right = dfc_right.loc[labels_right].apply(lambda r: r[col1] if r[col1] >= r[col2] else r[col2], axis=1)
            else: # midpoint
                y_coords_right = (dfc_right.loc[labels_right, col1] + dfc_right.loc[labels_right, col2]) / 2

            _label_features(ax=ax, x_coords=x_coords_right, y_coords=y_coords_right, 
                            labels_to_plot=labels_right, font_scale=font_scale)
