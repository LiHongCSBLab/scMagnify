...
def get_acts(
    ...
    if method == "aucell":
        estimate = run_aucell(
            adata, net, source=source, target=target, n_up=n_up, min_n=min_n, seed=seed, verbose=verbose, use_raw=use_raw
        )
    elif method == "mlm":
        estimate, pvals = run_mlm(
            adata, net, source=source, target=target, weight=weight, batch_size=batch_size, min_n=min_n, verbose=verbose, use_raw=use_raw
        )
    else:
...
def run_mlm(mat, net, source='source', target='target', weight='weight', batch_size=10000,
            min_n=5, verbose=False, use_raw=False):
    """
