...
def build_metacells_SEACells(
    ...
    use_raw: bool = False,
    rna_layer: Optional[str] = "counts",
    atac_layer: Optional[str] = "counts",
    n_metacells: Optional[int] = None,
    min_iter: int = 10,
    max_iter: int = 50,
    use_gpu: bool = False,
    groupby: Optional[str] = "celltype",
    mask_key: Optional[str] = "cell_state_masks",
    embed_key: Optional[str] = "X_umap",
    t_key: Optional[str] = "palantir_pseudotime",
) -> MuData:
    """
    ...
    if groupby is not None:
        ...
        top_group = rna_adata.obs[groupby].groupby(rna_adata.obs["SEACell"]).value_counts().groupby(level=0, group_keys=False).head(1)
    ...
    if mask_key is not None:
        ...
        meta_state_masks = (rna_adata.obsm[mask_key].assign(SEACell=rna_adata.obs["SEACell"]).groupby("SEACell").mean() > 0.5)
    ...
    if embed_key is not None:
        ...
        meta_mdata["RNA"].obsm[embed_key] = meta_embed.groupby(rna_adata.obs["SEACell"]).mean().loc[meta_mdata["RNA"].obs_names, :].values

    if t_key is not None:
        logg.info(f"Assigning [bright_cyan]{t_key}[/bright_cyan] to metacells...")      
        meta_mdata["RNA"].obs[t_key] = rna_adata.obs[t_key].groupby(rna_adata.obs["SEACell"]).mean().loc[meta_mdata["RNA"].obs_names].values
...
def _add_atac_meta_data(
    ...
    atac_meta_adata.var["GC_bin"] = np.digitize(
        atac_adata.var["GC"], np.linspace(0, 1, n_bins_for_gc)
    )
...
def build_metacells(
    ...
    for batch in batches:
        ...
        rna_pool = pd.DataFrame(rna_pool, 
                                index=rna_batch_adata.obs_names,
                                columns=rna_batch_adata.var_names)
        atac_pool = _pooling(atac_batch_adata.raw.X.toarray(), connectivities, degree)
        atac_pool = pd.DataFrame(atac_pool, 
                                index=atac_batch_adata.obs_names,
                                columns=atac_batch_adata.var_names)
