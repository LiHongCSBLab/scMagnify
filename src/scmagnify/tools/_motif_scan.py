...
from scmagnify.utils import _get_data_modal, _list_to_str, d, inject_docs
...
__all__ = ["MotifScanner",  
           "parse_pfm", "parse_jaspar", "parse_meme",
           "write_meme", "write_jaspar", "write_pfm",
...
def _add_peak_info(
    adata: AnnData, 
    peak_selected: Optional[List[str]] = None,
    peak_sequences: Optional[Dict[str, str]] = None,
): 
    """
    ...
    for peak in peak_selected:
        try:
            ...
            peak_info.append({
                "peak": peak, "seqnames": chrom, "start": start, "end": end,
                "width": width, "GC": gc_content, "N": n_content
            })
...
@d.dedent
def parse_jaspar(file_path: str,
                 source: Optional[str] = None) -> List[PFM]:
    """
            ...
            for line_number, line in enumerate(file, 1):
                ...
                if line.startswith('MOTIF'):
                    ...
                    try:
                        ...
                        for nuc, value in zip(nucleotide_order, values):
...
@d.dedent
def convert_motif_format(input_path: str, 
                         output_path: str, 
                         from_format: Literal["meme", "jaspar", "pfm"], 
                         to_format: Literal["meme", "jaspar", "pfm"]) -> None:
    """
    ...
    readers = {
        "meme": lambda p: parse_meme(p, to_df=True),
        "jaspar": parse_jaspar,
        "pfm": parse_pfm
    }
    writers = {
...
def filter_motifs_by_score(motif_score_df: pd.DataFrame, 
...
class MotifScanner:
    ...
    def __init__(self,
                 ...
                 motif_objects: Optional[List[PFM]] = None, 
    ...
    def import_motifs(self, 
                    motif_db: str = "HOCOMOCOv11_HUMAN",
                    format: Optional[Literal["meme", "jaspar", "pfm"]] = None, 
        ...
        if is_basename:
            logg.debug(f"'{motif_db}' has no extension. Assuming it's a PFM database name from the default motif directory.")
            db_name = motif_db
        ...
        if factor_list:
            ...
            filtered_motifs = [
                motif for motif in new_motifs 
                if motif.name in factors_to_keep
            ]
        ...
        if duplicate_ids:
            logg.warning(f"Skipped {len(duplicate_ids)} duplicate motifs with the following IDs: {', '.join(duplicate_ids)}")

        if target_organism:
            import decoupler as dc
            factor_df = pd.DataFrame(
                [motif.name for motif in unique_new_motifs], 
                columns=['factor']
            )
    ...
    def export_motifs(
        self, 
        output_path: str, 
        format: Literal["meme", "jaspar", "pfm"],
        **kwargs
    ) -> None:
        """
    ...
    def match(
        ...
        with Progress() as progress:
            ...
            else: _bg = MOODS.tools.flat_bg(4)
