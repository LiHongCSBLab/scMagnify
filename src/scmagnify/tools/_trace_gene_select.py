...
def plot_pipeline_status(df: pd.DataFrame):
    ...
    for i, (gene_name, row_data) in enumerate(df.iterrows()):
        y = y_coords[i]
...
def trace_gene_select(
    ...
    high_score_motifs_tfs = motif_scan["motif_score"].query(
        'score > @motif_scan["params"]["threshold"]', engine='python'
    )["motif2factors"].unique()
    ...
    significant_corr_genes = peak_gene_corrs["filtered_corrs"].query(
