...
def _parse_gmt(gmt_file_path: str, encoding: str = 'utf-8') -> pd.DataFrame:
    ...
    net_list = []
    try:
        with open(gmt_file_path, 'r', encoding=encoding) as f:
            for line in f:
        ...
        raise IOError(f"An error occurred while parsing '{gmt_file_path}': {e}")
...
def _p_adjust_fdr(p: Union[np.ndarray, list]) -> np.ndarray:
...
def _mlnTest2r(a, ab, ac, abcd):
    if 0 > a or a > ab or a > ac or ab + ac > abcd + a: return 0.0 
...
class FuncEnrich:
    ...
    def __init__(
        self,
        gene_sets: Union[str, pd.DataFrame],
        ...
        if self.net.empty: raise ValueError("The provided gene sets are empty or could not be parsed.")
        n_sets = self.net['geneset'].nunique()
        n_genes = self.net['genesymbol'].nunique()
        logg.info(f"Successfully loaded [bold green]{n_sets}[/bold green] gene sets with a total of [bold green]{n_genes}[/bold green] unique gene symbols.")
    ...
    def get_overlap_genes(self, 
                          terms: List[str], 
                          sortby: Optional[pd.DataFrame] = None,
                          n_top: Optional[int] = 5) -> Dict[str, List[str]]:
        """
        ...
        if sortby is not None and n_top is not None and n_top > 0:
            ...
            for term in overlap_dict.keys():
                ...
                sorted_genes = filtered_sortby.sort_values(by=filtered_sortby.columns[0], ascending=False).head(n_top).index.tolist()
    ...
    def add_genesets(
        self,
        new_sets: Union[dict, pd.DataFrame],
        geneset_col: str = 'geneset',
        genesymbol_col: str = 'genesymbol',
        inplace: bool = True
    ) -> Optional[FuncEnrich]:
        ...
        if isinstance(new_sets, dict):
            ...
            new_df = new_sets[[geneset_col, genesymbol_col]].rename(columns={
                geneset_col: 'geneset',
                genesymbol_col: 'genesymbol'
            })
        ...
        n_sets = self.net['geneset'].nunique()
        n_genes = self.net['genesymbol'].nunique()
        logg.info(f"Successfully added/updated gene sets. Total is now [bold green]{n_sets}[/bold green] sets with [bold green]{n_genes}[/bold green] unique genes.")
    ...
    def run_ora(
        self,
        gene_list: Union[list, pd.Series, pd.Index],
        ...
        if self.net.empty:
            logg.error("Cannot run ORA because no gene sets are loaded. The list might be empty due to prior filtering.")
            return pd.DataFrame()
        ...
        if n_background is None:
            ...
            logg.info(f"Using all [bold blue]{n_background_size}[/bold blue] genes from the loaded gene sets as background.")
            original_sig_count = sig_genes.size
            sig_genes = sig_genes[np.isin(sig_genes, background_from_net)]
            if sig_genes.size < original_sig_count: logg.warning(f"{original_sig_count - sig_genes.size} genes from your list were not in the background and were excluded.")
        ...
        if sig_genes.size == 0: raise ValueError("No genes from your list overlap with the background gene universe.")
        ...
        sizes, overlap_r, odds_r, pvals, overlaps = _get_pvals(
            sample=sig_genes_idx, net=net_flat, starts=starts, offsets=offsets, 
            n_background=n_background_size, n_table=len(all_genes_universe)
        )
        ...
        for i in range(terms.size):
            if overlap_r[i] > 0:
                ...
                res.append([terms[i], int(sizes[i]), f"{int(overlaps[i].sum())}/{int(sizes[i])}", overlap_r[i], pvals[i], odds_r[i], overlap_gene_names])
        
        if not res:
            logg.warning("Analysis complete. No significant enrichment found.")
            return pd.DataFrame()
            
        res_df = pd.DataFrame(res, columns=['Term', 'Set size', 'Overlap', 'Overlap ratio', 'p-value', 'Odds ratio', 'Features'])
        ...
        console = Console()
        table = Table(title=f"Top {min(top_n_results, len(results_sorted))} Enriched Terms", show_header=True, header_style="bold magenta")
