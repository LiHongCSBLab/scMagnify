...
class RegDecomp:
    ...
    def __init__(
        ...
        for attri in self.network.columns[2:-1]:
            self.filtered_network[attri] = filter_network(self.network, 
                                                        attri = attri,
                                                        method=filter[0],
                                                        param=filter[1],
                                                        binarize=filter[2],
        ...
        self.tensor = _network_to_tensor(self.filtered_network,
    ...
    def tucker_decomposition(
            self,
            rank: Union[int, tuple, list] = None,
            non_negative: bool = False,
        ...
        **kwargs
        ...
        if non_negative:
            ...
            logg.info(f"RegFactor Decomposing with Tucker decomposition: \n Tensor shape: {tensor.shape} \n Rank: {rank}")
    ...
    def compute_activity(
        ...
        if self.regfactor_key not in self.data.uns:
            raise ValueError(f"RegFactor key {self.regfactor_key} not found in data.uns. \n Run tensor decomposition first.")
    ...
    def rename_regfactors(
        ...
        sort: Union[bool, List[str], Literal['alphabetical']] = False,
...
def _network_to_tensor(edges_df: pd.DataFrame, 
                        tf_names: List[str], 
                        tg_names: List[str],
                        mode: str = "multiscale") -> np.ndarray:
    """
...
def extract_regfactor_genes(
    data: Union[AnnData, MuData, GRNMuData], # Simplified for demonstration
    regfactor_key: str = "regfactors",
    mode: str = "TF",
    threshold: float = 0.0,
    n_top: Optional[int] = None,
    percentile: Optional[float] = None, # New parameter
    plot: bool = False,
    ncols: int = 3,
