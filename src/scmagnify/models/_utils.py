...
__all__ = ["normalize_data", "partial_ordering", "chromatin_constraint", "import_basalGRN", "get_edgedf", "to_tensor", "to_nx"]
...
def partial_ordering(
    adata: AnnData,
    dyn: Literal['velocity', 'pseudotime'] = 'velocity',
    lag: int = 5
) -> torch.FloatTensor:
    """
...
def chromatin_constraint(
    ...
    basal_grn_filtered = basal_grn.loc[basal_grn["TF"].isin(regulators) & basal_grn["Target"].isin(targets), :].reset_index(drop=True)
    ...
    chrom_constraint = _edge_to_matrix(basal_grn_filtered, 
...
def import_basalGRN(
    ...
    if not set(["TF", "Target"]).issubset(basal_grn.columns):
    ...
    basal_grn_filtered = basal_grn.loc[basal_grn["TF"].isin(regs) & basal_grn["Target"].isin(Target), :].reset_index(drop=True)
...
def get_edgedf(
    ...
    idx = list(zip(idx_reg, idx_target))
    
    # Create the initial edges list DataFrame
    edges_df = pd.DataFrame(
        {'TF': tf_names[idx_reg], 'Target': tg_names[idx_target], 
...
def to_tensor(edges_df: pd.DataFrame, tf_names: List[str], tg_names: List[str]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
